...
puts "Creating Categories"
 ...
 puts "Creating Vegetables"
    ...
    veggies.each {|veg| Product.create!(name: veg, price: 10, stock: 10, unit_type: "kg", producer: Producer.find_by(name: farmers.sample), category: Category.find_by(name: "vegetables"))}
  puts "Vegetables made"

 puts "Creating Fruits"
    fruits = ["Strawberries", "Oranges", "Apples", "Grapes", "Bananas", "Mango", "Pineapple", "Melon"]
    fruits.each {|fruit| Product.create!(name: fruit, price: 10, stock: 10, unit_type: "kg", producer: Producer.find_by(name: farmers.sample), category: Category.find_by(name: "fruits"))}
  puts "Fruits made"

 puts "Creating Meats"
    meats = ["Beef", "Poultry", "Sausage", "Pork", "Lamb and mutton", "Duck", "Crocodile"]
    meats.each {|meat| Product.create!(name: meat, price: 10, stock: 10, unit_type: "kg", producer: Producer.find_by(name: farmers.sample), category: Category.find_by(name: "meat"))}
  puts "Meats made"

 puts "Creating Fish"
    fishes = ["Salmon", "Tuna", "Cod", "Catfish", "Octopus", "Bass", "Crab Legs", "Swordfish"]
    fishes.each {|fish| Product.create!(name: fish, price: 10, stock: 10, unit_type: "kg", producer: Producer.find_by(name: farmers.sample), category: Category.find_by(name: "fish"))}
  puts "Fish made"

puts "Creating Dairy & Eggs"
    dairies = ["Eggs", "Milk", "Cheese", "Yogurt", "Butter", "Ice-Cream", "Sour Cream", "Cream Cheese"]
    dairies.each {|dairy| Product.create!(name: dairy, price: 10, stock: 10, unit_type: "kg", producer: Producer.find_by(name: farmers.sample), category: Category.find_by(name: "dairy & eggs"))}
...
puts "Creating Bakery"
    ...
    bakes.each {|bake| Product.create!(name: bake, price: 10, stock: 10, unit_type: "kg", producer: Producer.find_by(name: farmers.sample), category: Category.find_by(name: "bakery"))}
  puts "Bakery made"

  puts "Creating Wine & Drinks"
    drinks = ["Red Wine", "White Wine", "Beer", "Non-alcoholic Beer", "Juices", "Teas", "Coffee", "Protein Shakes"]
    drinks.each {|drink| Product.create!(name: drink, price: 10, stock: 10, unit_type: "kg", producer: Producer.find_by(name: farmers.sample), category: Category.find_by(name: "wine & drinks"))}
  puts "Wine & Drinks made"

  puts "Creating Pantry"
    pantries = ["Pasta", "Flour", "Wheat", "Cereal", "Spices", "Oils", "Rice", "Cous Cous"]
    pantries.each {|pantry| Product.create!(name: pantry, price: 10, stock: 10, unit_type: "kg", producer: Producer.find_by(name: farmers.sample), category: Category.find_by(name: "pantry"))}
  puts "Pantry made"

  puts "Creating Deli"
    delis = ["Potatoe Salad", "Roasted Squash", "Roast Beef", "Green Bean Soup", "Saffron Rice", "Pad Thai", "Smoked Turkey", "Mac & Cheese"]
    delis.each {|deli| Product.create!(name: deli, price: 10, stock: 10, unit_type: "kg", producer: Producer.find_by(name: farmers.sample), category: Category.find_by(name: "deli"))}
  puts "Deli made"
